...
const Header = () => {
  const products = useSelector((state) => state.MarketReducer.products);
  const userInfo = useSelector((state) => state.MarketReducer.userInfo);
  const ref = useRef();
  ...
  return (
    <div className="sticky top-0 z-50">
      <div className="w-full bg-[#1c1c1c] text-white px-4 py-3 flex md:justify-between items-center gap-2 md:gap-4 lgl:gap-2 xl:gap-4">
        {/* ===================== Header Image Start here ======================== */}
        <Link to="/">
          <div className="headerHover">
            <img className="w-24 mt-2" src={logo} alt="logoImage" style={{ width: "9rem" }} />
          </div>
        </Link>
        ...
        <div className="hidden lgl:inline-flex h-10 rounded-md flex-grow relative">
          ...
          <input
            ...
            type="text"
          />
          <span className="w-12 h-full flex items-center justify-center bg-[#2121fa] hover:bg-[#2121fadb] duration-300 text-amazon_blue cursor-pointer rounded-tr-md rounded-br-md">
            <SearchIcon />
          </span>
        </div>
        ...
        <Link to="/cart">
          <div className="flex items-start justify-center headerHover relative">
            ...
            <span className="absolute text-xs top-0 left-6 w-4 font-semibold p-1 h-4 bg-[#2121fa] text-amazon_blue rounded-full flex justify-center items-center">
